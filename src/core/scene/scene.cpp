...
void SceneManager::loadAllRooms()
{
    for (int i = 0; i < this->roomFounds.size(); i++) {
        if (roomFounds[i].substr(roomFounds[i].find_last_of(".") + 1) == "gb") {
            ...
            registry->add_component<CPU>(newEntity, CPU{0x01, 0xB0, 0x00, 0x13, 0x00, 0xD8, 0x01, 0x4D, 0xFFFE, 0x0100, 0, true, {
                {0x00, [](CPU &cpu, Memory &mem) { /* NOP */ }},
                {0xC3, jp_C3},
                {0x20, jr_20},
                {0x01, ld_01}, {0x02, ld_02}, {0x11, ld_11}, {0x12, ld_12}, {0x21, ld_21}, {0x22, ld_22}, {0x31, ld_31}, {0x32, ld_32}, {0x3E, ld_3E}, {0x40, ld_40}, {0x41, ld_41}, {0x42, ld_42}, {0x0E, ld_0E}, {0x06, ld_06}, 
                ...
                {0xE0, ld_E0} , {0xF0, ld_F0},
                {0xAF, xor_AF},
                {0x05, dec_05}, {0x0D, dec_0D},
                {0xF3, di_F3},
            }});
        }
    }
